...
namespace CUSrinagar.WebApp.CUSrinagarAdminPanel.Controllers
{
    ...
    public class GrievanceController : AdminController
    {
        ...
        public ActionResult GrievanceList(Parameters parameter)
        {
            ...
            if (AppUserHelper.AppUsercompact.UserRoles.Contains(AppRoles.University_Dean) || AppUserHelper.AppUsercompact.UserRoles.Contains(AppRoles.University)
                || AppUserHelper.AppUsercompact.UserRoles.Contains(AppRoles.University_GrievanceFeedback)
                ) { }
            else
            {
        ...
        public ActionResult PostGrievanceReply(GrievanceReply model)
        {
            ...
            return RedirectToAction("Detail", "Grievance", new { @area = "CUSrinagarAdminPanel", @id = model.Grievance_ID });
        }
        ...
        public ActionResult AssignToMe(Guid? id)
        {
            ...
            SetResponseViewBags(response);
            return RedirectToAction("Detail", "Grievance", new { @area = "CUSrinagarAdminPanel", @id = id });
        }
        public ActionResult Discard(Guid? id)
        {
            ...
            if (id.HasValue)
            {
                response = new GrievanceManager().Discard(id.Value);
            }
            SetResponseViewBags(response);
            return RedirectToAction("Detail", "Grievance", new { @area = "CUSrinagarAdminPanel", @id = id });
        }
