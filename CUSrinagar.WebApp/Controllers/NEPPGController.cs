...
{
    public class NEPPGController : Controller
    {
        ...
        short MaximumNumber_Of_Course_DDL_Options { get { return 5; } }//5
        ...
        public ActionResult PersonalInfo(string r)
        {
            ...
            if (!r.IsNullOrEmpty() && Guid.TryParse(r.DecryptCookieAndURLSafe(), out _Student_ID))
                model = new RegistrationManager().GetStudentPersonalInfoOnly(Guid.Parse(r.DecryptCookieAndURLSafe()), PrintProgramme.PG, true);
            ...
            {
                if (new NEPManager().HasCookiePresent(PrintProgramme.PG, _Student_ID.ToString().EncryptCookieAndURLSafe()) == false)
                {
                    return RedirectToAction("Index", "NEP", new { area = "" });
                }
            }
            ...
            var RegistrationProcessClosed = new NEPManager().GetCourseListForRegistrationByGroup(PrintProgramme.PG, _Student_ID).IsNullOrEmpty();
            if (RegistrationProcessClosed)
            {
                var result = new NEPManager().GetNEPPersonalInfo(model.Student_ID, PrintProgramme.PG);
                if (result.Item1)
        ...
        public ActionResult PersonalInfo(ARGPersonalInformation model)
        {
            ...
            ARGFormNoMaster formNoMaster = new RegistrationManager().GetFormNoMaster(PrintProgramme.PG);

            if (formNoMaster.AllowNonCUET == false)
            {
                if (string.IsNullOrWhiteSpace(model.CUETApplicationNo))
                    ModelState.AddModelError("CUETApplicationNo", "CUET Application No. is required");
                if (string.IsNullOrWhiteSpace(model.CUETEntranceRollNo))
                    ModelState.AddModelError("CUETEntranceRollNo", "CUET Entrance RollNo. is required");
            }
            else
            {
                model.CUETApplicationNo = null;
                model.CUETEntranceRollNo = null;
            }
            ...
            else
            {
                ...
                {
                    Tuple<bool, string> _result = new NEPManager().Save(model, PrintProgramme.PG);
                    if (_result.Item1)
                    {
                        new NEPManager().CreateCookie(PrintProgramme.PG, model.Student_ID.ToString().EncryptCookieAndURLSafe());
        ...
        public ActionResult PreviousQualification(string r)
        {
            ...
            try
            {
                previousQualification = (ARGStudentPreviousQualifications)TempData["model"] ??
                         new RegistrationManager().GetStudentAcademicDetails(_Student_ID, PrintProgramme.PG)?.FirstOrDefault();
            }
            catch (Exception)
            {
                previousQualification = new RegistrationManager().GetStudentAcademicDetails(_Student_ID, PrintProgramme.PG)?.FirstOrDefault();
            }
        ...
        public ActionResult PreviousQualification(ARGStudentPreviousQualifications model, string IsProv)
        {
            try
            {
                ...
                {
                    new NEPManager().DeletePreviousQualifications(model.Student_ID, PrintProgramme.PG);
        ...
        public ActionResult SubjectPreference(string r)
        {
            ...
            ARGStudentPreviousQualifications previousQualification = new RegistrationManager().GetStudentAcademicDetails(_Student_ID, PrintProgramme.PG)?.FirstOrDefault();

            List<ARGCoursesApplied> listOfCoursesOpted = (List<ARGCoursesApplied>)TempData["model"] ?? new RegistrationManager().GetStudentCourses(_Student_ID, PrintProgramme.PG);

            foreach (ARGCoursesApplied item in listOfCoursesOpted ?? new List<ARGCoursesApplied>())
            {
                item.CUETEntrancePoints = Convert.ToDouble(item.SubjectEntrancePoints);
                item.CUETPercentileScore = item.SubjectCGPA;
            }
        ...
        public ActionResult SubjectPreference(List<ARGCoursesApplied> model)
        {
            ...
            if (!model.IsNullOrEmpty())
            {
                ...
                if (formNoMaster.AllowNonCUET == false)
                {
                    if (model.Any(x => x.Course_ID != Guid.Empty && x.CUETEntrancePoints == null))
                    {
                        TempData["ErrorMessage"] = errorMsg.Replace("##", "Please fill all required fields");
                ...
                if (ModelState.IsValid)
                {
                    ...
                    Tuple<bool, string> _result = new NEPManager().Save(model, PrintProgramme.PG, formNoMaster.AllowNonCUET);
                    if (_result.Item1)
                    {
                        TempData["SuccessMessage"] = "Details saved successfully.";
        ...
        public ActionResult DocumentUpload(string r)
        {
            ...
            NEPDocuments nepDocuments = new NEPManager().GetDocuments(_Student_ID, PrintProgramme.PG);
            ViewBag.HasDocuments = nepDocuments != null;
            NEPDocuments model = FillDocumentPreference_ViewBags(_Student_ID, nepDocuments, PrintProgramme.PG);
            return View(model);
        }
        ...
        public ActionResult MakePayment(string r)
        {
            ...
            string isEligileForPayment = new NEPManager().IsEligibleForPayment(_Student_ID, PrintProgramme.PG);
            ...
            ViewBag.PersonalInfoCompact = new StudentManager().GetStudentC(_Student_ID, PrintProgramme.PG);

            PaymentDetails payment = new PaymentManager().GetPaymentDetails(_Student_ID, PaymentModuleType.Admission, PrintProgramme.PG)?.FirstOrDefault();
        ...
        public ActionResult PaymentResponse()
        {
            ...
            if (billdeskResponse.Item1)
            {
                ...
                var result = new NEPManager().Save(billdeskResponse.Item3, PrintProgramme.PG);
                if (result.Item1)
                ...
                else
                {
                    ...
                    return RedirectToAction("MakePayment", "NEPPG", new { r = Student_ID.ToString().EncryptCookieAndURLSafe() });
                }
            }
            else
            {
        ...
        public async Task<ActionResult> Print(string r)
        {
            ...
            #region Block Edit
            bool blockEdit = new NEPManager().BlockEditAfterPayment(_Student_ID, PrintProgramme.PG);
            if (!blockEdit)
            {
                if (new NEPManager().HasCookiePresent(PrintProgramme.PG, _Student_ID.ToString().EncryptCookieAndURLSafe()) == false
            ...
            #endregion
            var result = await new NEPManager().GetNEPPersonalInfoAsync(_Student_ID, PrintProgramme.PG);
            ...
            ViewBag.courseApplied = NEPProgrammesAdmTitle.Post_Graduation_Programmes.GetEnumDescription();
        ...
        private ARGPersonalInformation FillPersonalInfo_ViewBags(ARGPersonalInformation personalInfo)
        {
            ...
            ARGFormNoMaster formNoMaster = new RegistrationManager().GetFormNoMaster(PrintProgramme.PG);
            ViewBag.AllowNonCUET = formNoMaster.AllowNonCUET;
            ViewBag.isSelfFinanceForm = formNoMaster.AllowApplyForSelfFinancedSeat;
        ...
        private ARGStudentPreviousQualifications FillPreviousQualification_ViewBags(Guid _Student_ID, ARGStudentPreviousQualifications previousQualification)
        {
            ...
            ViewBag.University = Helper.UniversitiesDDL();
        ...
        private List<ARGCoursesApplied> FillSubjectPreference_ViewBags(Guid _Student_ID, List<ARGCoursesApplied> listOfCoursesOpted, ARGStudentPreviousQualifications previousQualification)
        {
            ...
            if (listOfCoursesOpted.IsNullOrEmpty())
            {
                ...
                {
                    model.Add(new ARGCoursesApplied() { Student_ID = _Student_ID, Preference = index, IsActive = true, PrintProgramme = PrintProgramme.PG, Programme = Programme.PG });
                }
            }
            else
            {
                ...
                if (number_Of_DDL_Options > 0)
                {
                    ...
                    {
                        model.Add(new ARGCoursesApplied()
                        {
                            Student_ID = _Student_ID,
                            Preference = (short)(maxPrefenceOrder + index),
                            IsActive = true,
            ...
            List<DropDownOptLabelGeneral> CoursesDDL = new NEPManager().GetCourseListForRegistrationByGroup(PrintProgramme.PG, _Student_ID);
            ...
            if (previousQualification != null)
            {
                removeMathEligigleCourses =
                      previousQualification.Subjects.ToUpper().Contains("MATH")
                     || previousQualification.Subjects.ToUpper().Contains("BCA")
                     || previousQualification.Subjects.ToUpper().Contains("COMPUTER")
                     || previousQualification.Subjects.ToUpper().Contains("INFORMATION TECHNOLOGY");

                if (!removeMathEligigleCourses && CoursesDDL.IsNotNullOrEmpty())
                {
                    //remove math eligible courses
                    int indexOfComputerApp = CoursesDDL.FindIndex(x => Guid.Parse(x.Value.ToString()) == Guid.Parse("5C1695B0-5485-4ACA-B6C4-FA264BF34303"));/*MCA*/
                    if (indexOfComputerApp != -1)
                        CoursesDDL.RemoveAt(indexOfComputerApp);

                    int indexOfIT = CoursesDDL.FindIndex(x => Guid.Parse(x.Value.ToString()) == Guid.Parse("3E887977-D3F0-405E-BA1B-018CAD4D2D83")); /*IT*/
                    if (indexOfIT != -1)
            ...
            ViewBag.ErrorMessage = TempData["ErrorMessage"];
            ViewBag.SuccessMessage = TempData["SuccessMessage"];
            ViewBag.FormNoMater = new RegistrationManager().GetFormNoMaster(PrintProgramme.PG);
            return model;
        }
        ...
        {
            List<ARGCoursesApplied> listOfCoursesOpted = new RegistrationManager().GetStudentCourses(_Student_ID, PrintProgramme.PG);
            if (listOfCoursesOpted.IsNullOrEmpty())
            {
            ...
            ARGPersonalInformation stdInfo = new RegistrationManager().GetStudentPersonalInfoOnly(_Student_ID, PrintProgramme.PG);
        ...
        {
            ARGPersonalInformation stdInfo = new RegistrationManager().GetStudentPersonalInfoOnly(_Student_ID, PrintProgramme.PG, true);
            ARGFormNoMaster formNoMaster = new RegistrationManager().GetFormNoMaster(PrintProgramme.PG);
        ...
        {
            ARGFormNoMaster formNoMaster = new RegistrationManager().GetFormNoMaster(PrintProgramme.PG) ?? new ARGFormNoMaster();
            BillDeskRequest billDeskRequest = new BillDeskRequest()
            {
                ...
                Semester = "0",
                ReturnURL = "NEPPG/PaymentResponse",
