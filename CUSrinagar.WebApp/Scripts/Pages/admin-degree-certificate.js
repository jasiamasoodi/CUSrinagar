...
$(document).ready(function () {
    ...
    $(document).on('click', '.jsValidated', function () {
        var $element = $(this);
        var degreecertificate_id = $element.val();
        var _url = "/CUSrinagarAdminPanel/" + "DegreeCertificate/Validated";
        print_Validate(_url, degreecertificate_id, $element);
    });
    ...
    $(document).on('click', '.jsDuplicate', function () {
        ...
        DuplicaDegree(_url, degreecertificate_id, duplicatelbl, $element);
    });
    ...
    $(document).on('click', '.jsShowDegreeCertificateDialog', function () {
        ...
        $("#jsALLDegreeCertificateBtn").trigger("click");
    ...
    $(document).on('click', '#jsDegreeCertificateBtn', function () {
        ...
        $.ajax({
            ...
            datatype: "Json",
            async: false,
            data: { DegreeCertificate_ID: student_id, CommonDispatchNumber: _CommonDispatchNumber, StartingIssueNumber: _StartingIssueNumber, PrintProgramme: $("#PrintProgramme").val() },
    ...
    $(document).on('click', '#jsALLDegreeCertificateBtn', function () {
        showLoader();
        ...
        var _url = "/CUSrinagarAdminPanel/" + "DegreeCertificate/CreateALL";
        ...
        $.ajax({
            ...
            datatype: "Json",
            async: false,
            data: { Student_ID: list, SemesterFrom: $("#SemesterFrom").val(), SemesterTo: $("#SemesterTo").val() , Batch: _Batch, PrintProgramme: $("#PrintProgramme").val() },
            success: function (responseData) {
                hideLoader();
                if (responseData.IsSuccess) {
                    showSuccessAlertMessage(responseData.NumberOfRecordsEffected + " record updated successfully.", 300);
...
$(document).on('click', '#jsALLPrintedBtn', function () {
    ...
    if (list.length <= 0) {
        ...
        $.ajax({
            ...
            datatype: "Json",
            async: false,
            data: { DegreeCertificate_IDs: list },
            success: function (responseData) {
                if (responseData.IsSuccess) {
                    showSuccessAlertMessage(responseData.NumberOfRecordsEffected + " record updated successfully.", 300);
                    hideLoader();
...
function DuplicaDegree(_url, student_id, duplicatelbl, $element) {
    $.ajax({
        ...
        datatype: "Json",
        async: false,
        data: { DegreeCertificate_ID: student_id, DuplicateLbl: duplicatelbl, PrintProgramme: $("#PrintProgramme").val() },
        success: function (responseData) {
            if (responseData.IsSuccess) {
                showSuccessAlertMessage(responseData.NumberOfRecordsEffected + " record updated successfully.", 300);
                $("body").find(".js-search").trigger("click");
...
function print_Validate(_url, degreecertificate_id, $element) {
    $.ajax({
        url: _url,
        type: "POST",
        datatype: "Json",
        async: false,
        data: { DegreeCertificate_ID: degreecertificate_id, PrintProgramme: $("#PrintProgramme").val() },
        success: function (responseData) {
            if (responseData.IsSuccess) {
                showSuccessAlertMessage(responseData.NumberOfRecordsEffected + " record updated successfully.", 300);
                $("body").find(".js-search").trigger("click");
